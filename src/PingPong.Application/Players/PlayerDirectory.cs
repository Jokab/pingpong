...
public sealed class PlayerDirectory : IPlayerDirectory
{
    ...
    )
    {
        ...
        var candidates = await _context
            .Players.Where(player =>
                player.NormalizedName.Contains(normalized)
                || EF.Functions.Like(player.DisplayName, $"%{trimmed}%")
            )
            .Select(p => new
            {
                p.Id,
        ...
        var results = candidates
            .Select(p =>
            {
                ...
                var isExactInsensitive = string.Equals(
                    p.DisplayName,
                    trimmed,
                ...
                return new PlayerSearchResult(
                    ...
                    p.DisplayName,
                    score,
                    isExactInsensitive || p.NormalizedName == normalized
                );
            })
    ...
    )
    {
        ...
        var existsSameDisplay = await _context.Players.AnyAsync(
            p => p.DisplayName.ToUpper() == trimmed.ToUpper(),
            cancellationToken
        );
        ...
        if (existsSameDisplay)
        {
            ...
            for (var i = 0; i < 30; i++)
            {
                ...
                var taken = await _context.Players.AnyAsync(
                    p => p.DisplayName.ToUpper() == candidate.ToUpper(),
                    cancellationToken
                );
                if (!taken)
                {
    ...
    public async Task<Player> EnsurePlayerAsync(
        string displayName,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var existing = await _context.Players.FirstOrDefaultAsync(
            player => player.NormalizedName == normalizedKey,
            cancellationToken
        );
